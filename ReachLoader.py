...
import skvideo.io
# Global variables
trial_classifier_path = 'data/models/null_model.joblib'
hand_classifier_path = 'data/models/numReach_model.joblib'
num_reach_classifier_path = 'data/models/whichHand_model.joblib'
...
class ReachViz:
    ...
    def segment_reaches_with_speed_peaks(self,p_filter_val=0.4, pos_filter_val=0.14):
        ...
        self.left_palm_maxima = find_peaks(lps, height=0.2, distance=40)[0]
        self.right_palm_maxima = find_peaks(rps, height=0.2, distance=40)[0]
    ...
    def plot_predictions_videos(self, segment=False, clean=False):
        ...
        for isx in range(tlde, tldr):
            ...
            axel1.plot(frames[0:isx], self.lick_vector[0:isx], label='Licking Activity')
            axel2.plot(frames[0:isx], self.lick_vector[0:isx])
            if self.behavior_start_time is not None:
                for idx, reaches in enumerate(self.individual_reach_times):
